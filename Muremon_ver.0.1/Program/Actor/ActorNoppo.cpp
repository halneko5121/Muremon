 ...
 */
void
C_Chara_Noppo::Control(int key, POS_CC<float> boss_cc, int sound_startnum, int rect_startnum,bool boss_death)			//キャラクタの制御
{
	m_randspeed = 0.f;
		...
		if(m_charadata[i].flag_hit){
			...
			if(!m_charadata[i].flag_effectfont){
				if(m_count_effect[i]++ < FONT_SET){
					pos_effectfont[i] = SetE_Font(m_charadata[i].draw_cc, RADIUS_NOPPO,POS_HITFONT_X);
 ...
 */
POS_CC<float>
C_Chara_Noppo::CharaAttack_2(int m_chara_num)																//キー入力による動作その2
{
 ...
 */
void
C_Chara_Noppo::DeathControl(int m_chara_num, int sound_startnum ,int rect_startnum)							//死亡処理
{
