...
//キャラクタの制御
void C_Chara_Yoshi::Control(int key,  POS_CC<float> boss_cc, int sound_startnum, int rect_startnum, bool boss_death)
{
	m_randspeed = 0.f;
...
int C_Chara_Yoshi::SetAnimetion(int max_animetion, int anime_count ,int rect_num ,int m_chara_num)
{
	static int delay = 0;
...
void C_Chara_Yoshi::DrawEffectFont(int t_num, int rect_startnum,LPDIRECT3DDEVICE9 device_data)
{
	int rect_change = 0;
...
void C_Chara_Yoshi::Draw(int t_num, int rect_startnum, LPDIRECT3DDEVICE9 device_data)
{
	//キャラの描画(いちお100体分)
	for(int i = 0;i < MAX_VALLUE_PLAYER;i++){
		if(m_charadata[i].flag_atk1){
...
POS_CC<float> C_Chara_Yoshi::CharaAttack_2(int m_chara_num)														//キー入力による動作その2
{
...
void C_Chara_Yoshi::DeathControl(int m_chara_num, int sound_num, int rect_startnum)						//死亡処理
{
